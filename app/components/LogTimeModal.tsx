...
export default function LogTimeModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleLog = async () => {
    // TODO: save to time_logs table
    console.log({ case_id: selectedCase, activity: selectedActivity, hours });
    alert('Time logged for selected case!');
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black/80 flex items-end z-50">
      <div className="bg-zinc-950 w-full rounded-t-3xl p-6 text-white">
        ...
        <div className="space-y-6">
          ...
          <button
            onClick={handleLog}
            className="w-full bg-white text-black py-5 rounded-3xl text-xl font-medium mt-8"
          >
            Log Time
          </button>
        </div>
