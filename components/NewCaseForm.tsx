...
export default function NewCaseForm({ onSubmit, onClose, existingCase, onDelete }: NewCaseFormProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="bg-[#1C1C1E] w-full max-w-md rounded-t-3xl flex flex-col overflow-hidden max-h-[90dvh]"
        style={{
      ...
      >
        ...
        <div className="flex justify-between items-center px-6 pt-6 pb-3 flex-shrink-0">
          ...
          <button onClick={onClose} className="text-3xl leading-none text-white/70 active:text-white">×</button>
        </div>

        {/* Scrollable form area */}
        <div
          className="flex-1 overflow-y-auto min-h-0 px-6 pb-6 space-y-5"
          style={{
            // Extra bottom padding for safe area + to ensure last button is comfortable to tap
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Delete Button + Confirmation - only for editing. Now reachable via scroll. */}
            {existingCase && (
              <div className="pt-4">
                <button
                  type="button"
